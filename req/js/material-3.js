...
// Portfolio scripts (Manideep Pekety)
    ...
    chatToggle.addEventListener('click', () => {
        ...
        if (chatWidget.classList.contains('active')) {
            ...
            if (chatMessages && chatMessages.children.length === 0) {
                addChatMessage("Hi! I'm here to help you learn about Manideep's DevOps experience. Ask me anything!", 'bot');
            }
        }
    });
...
function generateBotResponse(userMessage) {
    ...
    if (message.includes('devops') || message.includes('experience')) {
        return "Manideep is a DevOps Engineer at Netenrich (Hyderabad) since September 2023. He builds on AWS and GCP with Terraform, Jenkins, Docker, Kubernetes (EKS/GKE), and strong CI/CD and observability practices.";
    }
    
    if (message.includes('cloud') || message.includes('aws') || message.includes('azure')) {
        return "Manideep works hands-on with AWS (EC2, S3, VPC, IAM, ASG, EKS, RDS, CloudWatch) and GCP (Compute Engine, GKE, Cloud Run, Functions, Pub/Sub), aligned with his resume.";
    }
    
    if (message.includes('automation') || message.includes('script')) {
        return "Automation is central to his role: Bash and Python scripting, Terraform for IaC, Jenkins and AWS CodePipeline for CI/CD, plus security hooks with Trivy and SonarQube.";
    }
    
    if (message.includes('award') || message.includes('recognition')) {
        return "Recognition is summarized in the Awards section on this site. For certifications, see AWS Cloud Practitioner, Azure AI Fundamentals, APSSDC Python, and NDG Linux Unhatched.";
    }
    
    if (message.includes('contact') || message.includes('hire') || message.includes('work')) {
        return "You can reach Manideep through the Contact section: manideep.pekety6@gmail.com and LinkedIn. He's based in Hyderabad, India.";
    }
    
    if (message.includes('skills') || message.includes('technology')) {
        return "Core stack from his resume: Docker, Kubernetes, Jenkins, Maven, AWS CodePipeline, Git/GitHub, Terraform, Prometheus, Grafana, Datadog, New Relic, ELK, Argo CD, Kafka, SonarQube, Trivy, Bash, Python.";
    }
    
    return "That's a great question! Explore Manideep's sections above or ask about Netenrich, AWS/GCP, certifications, or education (B.Tech CSE, Aditya College of Engineering).";
}
...
// Chat UI is handled by req/js/chatbot-fix.js (local Manideep assistant; no backend).
